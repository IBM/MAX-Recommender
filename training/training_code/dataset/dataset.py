...
class Dataset(object):
    ...
    def _reindex(self, df, binary):
        ...
        if binary:
            df[self.col_rating] = df[self.col_rating].apply(
                lambda x: float(x > 0))
    ...
    def _init_train_data(self):
        ...
        self.interact_status[self.col_item +
                             "_negative"] = self.interact_status[self.col_item +
                                                                 "_interacted"].apply(lambda x: self.item_pool -
                                                                                      x)
    ...
    def _init_test_data(self):
        ...
        if self.test is not None:
            ...
            test_interact_status = (
                self.test.groupby(
                    self.col_user)[
                    self.col_item] .apply(set) .reset_index() .rename(
                    columns={
                        self.col_item: self.col_item +
                        "_interacted_test"}))

            # get negative pools for every user based on training and test
            # interactions
            ...
            test_ratings = pd.merge(
                self.test,
                test_interact_status[[self.col_user,
                                      self.col_item + "_negative"]],
                on=self.col_user,
                how="left",
            )
            ...
            try:
                ...
                min_num = min(
                    map(len, list(test_ratings[self.col_item + "_negative"])))
                warnings.warn(
    ...
    def negative_sampling(self):
        ...
        try:
            ...
            min_num = min(
                map(len, list(train_ratings[self.col_item + "_negative"])))
            warnings.warn(
