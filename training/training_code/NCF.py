...
class NCF:
    ...
    def _create_model(self,):
        ...
        with tf.variable_scope("gmf", reuse=tf.AUTO_REUSE):
            ...
            self.gmf_p = tf.reduce_sum(
                tf.nn.embedding_lookup(
                    self.embedding_gmf_P, self.user_input), 1
            )
            self.gmf_q = tf.reduce_sum(
                tf.nn.embedding_lookup(
                    self.embedding_gmf_Q, self.item_input), 1
            )
        ...
        with tf.variable_scope("mlp", reuse=tf.AUTO_REUSE):
            ...
            self.mlp_p = tf.reduce_sum(
                tf.nn.embedding_lookup(
                    self.embedding_mlp_P, self.user_input), 1
            )
            self.mlp_q = tf.reduce_sum(
                tf.nn.embedding_lookup(
                    self.embedding_mlp_Q, self.item_input), 1
            )
        ...
        with tf.variable_scope("ncf", reuse=tf.AUTO_REUSE):
            ...
            if self.model_type == "gmf":
                ...
                self.ncf_vector = tf.concat(
                    [self.gmf_vector, self.mlp_vector], 1)
    ...
    def _load_neumf(self, gmf_dir, mlp_dir, alpha):
        ...
        vars_list = tf.get_collection(
            tf.GraphKeys.GLOBAL_VARIABLES, scope="ncf")
    ...
    def predict(self, user_input, item_input, is_list=False, is_mapped=True):
        ...
        if is_list:
            output = self._predict(user_input, item_input, is_mapped)
            return list(output.reshape(-1))

        else:
            output = self._predict(
                np.array([user_input]), np.array([item_input]), is_mapped)
            return float(output.reshape(-1)[0])

    def _predict(self, user_input, item_input, is_mapped):
